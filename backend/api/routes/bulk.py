...
async def validate_bulk_generation(
    ...
    """
    Перевірити можливість масового створення документів.

    Валідує список співробітників на наявність конфліктів дат та інших проблем.
    Це попередній крок перед генерацією.

    Parameters:
    - **request**: Список ID співробітників та дати.

    Returns:
    - Lists of **valid** (готові до створення) and **invalid** (з помилками) співробітників.
    """
...
async def generate_bulk_documents(
    ...
    """
    Масове створення документів (Execute).

    Створює документи для всіх 'valid' співробітників за один запит.
    Автоматично генерує файли .docx та записи в БД.

    Returns:
    - Статистика успішних створень та список ID нових документів.
    """
    from backend.models.staff import Staff
